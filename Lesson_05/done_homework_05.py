...
def find_cars(car_data, search_criteria):
  ...
  filtered_cars = [
    ...
    if details[1] >= year_min and details[2] >= engine_volume_min and details[4] <= price_max
  ]
  sorted_cars = sorted(filtered_cars, key=lambda x: x[1][4])

  for car, details in sorted_cars[:5]:
    print(f"{car}: Color: {details[0]}, Year: {details[1]}, Engine Volume: {details[2]}, Type: {details[3]}, Price: {details[4]}")
